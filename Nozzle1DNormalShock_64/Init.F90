...
!-------------------------------------------------
  ...
  subroutine init_alloc(N)
    ...
    do i = 1, N
        if(xp(i) >= Lc) then
           Raio(i) = rg + ((rin-rg)/2.0q0)*(1.0q0+cos(2.0q0*PI*(xp(i)-Lc)/Ln))
        else
           Raio(i) = rin
        end if
    end do
	...
	do i = 1, N-1
	   if(xe(i) >= Lc) then
	        Se(i) = Pi*((rg + ((rin-rg)/2.0q0)*(1.0q0+cos(2.0q0*PI*(xe(i)-Lc)/Ln)))**2.0q0)
	   else
            Se(i) = Pi*(rin**2.0q0)
       end if
	end do  
