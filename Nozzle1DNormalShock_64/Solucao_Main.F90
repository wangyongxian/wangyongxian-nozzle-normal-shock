...
subroutine MASSA
	...
	integer :: it
    real*16 :: k    ! auxiliar
    real*16 :: M_in ! número de Mach na entrada
    
	tcpu = timef() ! zera cronômetro
	
    ue = 0.0q0
    !p     = p_cam/((1.0q0+(gama-1.0q0)*(Mach**2)/2.0q0)**(gama/(gama-1.0q0)))
    ro    = p / ( R * T )
    ue    = u(1:n-1)
    pl    = 0.0q0
    roe   = ro(1:n-1)
    ...
    u_in  = ue(1)
    u_out  = ue(N-1)
    T_in  = T_cam - 0.5q0*(gama-1.0q0)*(u_in**2)/(gama*R)
    T_out  = T_cam - 0.5q0*(gama-1.0q0)*(u_out**2)/(gama*R)
    
    M_in = u_in / qsqrt ( gama * R * T_in )
    M_out = u_out / qsqrt ( gama * R * T_out )
    
    k = 1.0q0 + 0.5q0 * ( gama - 1.0q0 ) * ( M_in ** 2 )
    p_in = p_cam / ( k ** ( gama / ( gama - 1.0q0 ) ) )
    k = 1.0q0 + 0.5q0 * ( gama - 1.0q0 ) * ( M_out ** 2 )
    p_outN = p_out
    ...
    if (P_OUT /= -1.0q0) then
        p(n) = 2.0q0*p_out - p(n-1)  !2.0q0*p(n-1) - p(n-2)
    else
        p(n) = 2.0q0*p(n-1) - p(n-2)
    end if
    
    u(n)  = 2.0q0*u(n-1) - u(n-2)
   ! T(n)  = 2.0q0*T(n-1) - T(n-2)
    T(N) = -T(N-1) + 2*T_out
    ro(n) = p(n) / ( R * T(n) )
    de = 0.0q0
    ds = 0.0q0
    ...
    do it = 1, iteracao
      ...
      !if ((it > 5000).and.(it < 50000)) beta = 0.9q0
      !if (it >= 50000 ) beta = 1.0q0
      ...
      if (P_OUT /= -1.0q0) then
          ...
          M_out = u_out / qsqrt ( gama * R * T_out )
          k = 1.0q0 + 0.5q0 * ( gama - 1.0q0 ) * ( M_out ** 2 )
          p_outN = p_out / ( k ** ( gama / ( gama - 1.0q0 ) ) )
          p(N) = 2.0q0*p_outN - p(N-1)
...
subroutine correcoes_com_plinha
    ...
    ! velocidade nodal
    do i = 2,n-1
       u(i) = u(i) - 0.5q0 * ds(i) * ( pl(i+1) - pl(i-1) )
    end do
...
!-------------------------------------------------
  ...
  subroutine atualizar_ficticios_massa
	...
	pl(1) = 2.0q0*pl(1+1) - pl(1+2)

	! Calculando fictício P = N 
	pl(N) = 2.0q0*pl(N-1) - pl(N-2)    
...
!-------------------------------------------------
  ...
  subroutine corrigir_velocidades
	...
	do i = 2, N-1
	   u(i) = u(i) - ds(i)*(pl(i+1)-pl(i-1))/2.0q0
	end do
...
subroutine calculo_massa_especifica_nas_faces
    ...
    do i = 1, n-1
       ...
       roe(i) = ro(i) + beta * 0.5q0 * ( ro(i+1) - ro(i) )
