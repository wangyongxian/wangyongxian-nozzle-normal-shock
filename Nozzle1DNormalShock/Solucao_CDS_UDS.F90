   ...
   ! bc = 0.0d0
    ...
    do i = 2, n-1
       ...
       bp(i) = ro_o(i) * sp(i) * dx * u_o(i) / dt   &
             ...
             + 0.5d0 * beta * roe(i-1) * Se(i-1) * ue(i-1) * ( u(i) - u(i-1) )  &
             - 0.5d0 * beta * roe(i) * Se(i) * ue(i) * ( u(i+1) - u(i) )
  ...
  subroutine coeficientes_fontes_massa_cds_uds
    ...
    do i = 2, n-1
       ...
       bp(i) = -( ( ro(i)-ro_o(i) ) * sp(i) * dx / dt    &
               +ro(i) * ue(i) * se(i) &
               -ro(i-1) * ue(i-1) * se(i-1) &
               +0.5d0 * beta * ue(i) * se(i) * ( ro(i+1) - ro(i) ) &
               -0.5d0 * beta * ue(i-1) * se(i-1) * ( ro(i) - ro(i-1) ) )
